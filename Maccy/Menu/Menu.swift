...
class Menu: NSMenu, NSMenuDelegate {
    ...
    init(history: History, clipboard: Clipboard) {
        ...
        delegate = self
        minimumWidth = CGFloat(Menu.menuWidth)
    ...
    func latexify() {
        ...
        if let historyItemToRemove = itemToRemove as? HistoryMenuItem {
            ...
            guard let contents = historyItemToRemove.item?.getContents() else {
                return
            }
            for content in contents {
                latexifyTable(content: content)
            }
            updateUnpinnedItemsVisibility()
            setKeyEquivalents(historyMenuItems)
        }
    ...
    private func latexifyTable(content: HistoryItemContent) {
        var latex = ""
        let doc = try? XMLDocument(data: content.value)
        if doc != nil {
            let array = NSMutableArray()
            let table = doc!.firstChild(css: "table")
            let trs = table!.children(tag: "tr")
            var maxTdSize = 0
            for tr in trs {
                let tds = tr.children(tag: "td")
                maxTdSize = tds.count > maxTdSize ? tds.count : maxTdSize
                let elements = NSMutableArray()
                for td in tds {
                    let content = td.stringValue
                    elements.add(content)
                }
                let line = elements.componentsJoined(by: " & ")
                array.add(line)
            }
            let ths = NSMutableArray()
            for i in 0 ... maxTdSize {
                ths.add("c")
            }
            latex = #"""
            \begin{table}
                \begin{tabular}{|\#(ths.componentsJoined(by: "|"))|}
                \hline
